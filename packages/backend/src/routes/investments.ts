...
import { holdings, holdingTransactions, mortgages, properties, propertyTransactions } from "../db/schema";
...
app.post("/holdings", async (c) => {
  ...
  const [data] = await db.insert(holdings).values({ ...body, userId: user.id }).returning();
...
app.get("/holding-transactions", async (c) => {
  ...
  if (holdingId) {
    ...
    const data = await db
      ...
      .where(and(eq(holdingTransactions.holdingId, parsedHoldingId), eq(holdingTransactions.userId, user.id)));
    return c.json({ data });
  }
  const data = await db.select().from(holdingTransactions).where(eq(holdingTransactions.userId, user.id));
  return c.json({ data });
});
...
app.post("/holding-transactions", async (c) => {
  ...
  const [data] = await db.insert(holdingTransactions).values({ ...body, holdingId, userId: user.id }).returning();
  return c.json({ data }, 201);
});
...
app.get("/property-transactions", async (c) => {
  ...
  if (propertyId) {
    ...
    const data = await db
      ...
      .where(and(eq(propertyTransactions.propertyId, parsedPropertyId), eq(propertyTransactions.userId, user.id)));
    return c.json({ data });
  }
  const data = await db.select().from(propertyTransactions).where(eq(propertyTransactions.userId, user.id));
  return c.json({ data });
});
...
app.post("/property-transactions", async (c) => {
  ...
  const [data] = await db.insert(propertyTransactions).values({ ...body, propertyId, userId: user.id }).returning();
  return c.json({ data }, 201);
});
