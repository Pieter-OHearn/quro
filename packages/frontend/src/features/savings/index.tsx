...
type ChartsProps = {
  ...
  contribChartData: { month: string; contribution: number; interest: number; withdrawals: number }[];
  baseCurrency: string;
  fmtBase: (v: number, c?: unknown, d?: boolean) => string;
};
...
function AccountRowHeader({
  acc, accTxns, pct, foreign, balanceInBase, monthlyInterest, isExpanded,
  fmtBase, fmtNative, onToggleExpand, onEdit,
...
function AccountRow({
  acc, transactions, totalInBase, isExpanded,
  convertToBase, isForeign, fmtBase, fmtNative,
  onToggleExpand, onEdit, onAddTxn, onDeleteTxn,
}: AccountRowProps) {
...
function AccountsList({
  accounts, transactions, totalInBase, totalInterest, expandedId,
  convertToBase, isForeign, fmtBase, fmtNative,
  onToggleExpand, onEdit, onAddAccount, onAddTxn, onDeleteTxn,
}: AccountsListProps) {
  return (
...
export function Savings() {
  ...
  function handleSaveAccount(a: Omit<SavingsAccount, "id"> & { id?: number }): void {
    if (a.id) { updateAccount.mutate(a as SavingsAccount); } else { createAccount.mutate(a); }
  }
  function handleDeleteAccount(id: number): void { deleteAccount.mutate(id); }
  function handleAddTxn(t: Omit<SavingsTransaction, "id">): void { createTxn.mutate(t); }
  function handleDeleteTxn(id: number): void { deleteTxn.mutate(id); }
