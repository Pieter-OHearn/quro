...
export function AddMortgageModal({ existing, properties, linkedPropertyId, onClose, onSave }: AddMortgageModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
          ...
          <div>
            ...
            <select
              className={`w-full rounded-xl border px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 ${
                errors.linkedPropertyId ? "border-rose-300 bg-rose-50" : "border-slate-200 bg-slate-50"
              }`}
          ...
          {ltvPreview && (
            <div className={`rounded-xl p-4 border flex items-center justify-between ${parseFloat(ltvPreview) < 70 ? "bg-emerald-50 border-emerald-100" : parseFloat(ltvPreview) < 85 ? "bg-amber-50 border-amber-100" : "bg-rose-50 border-rose-100"}`}>
              <div>
                <p className="text-xs font-semibold text-slate-700">Loan-to-Value Preview</p>
                <p className="text-xs text-slate-500 mt-0.5">
                  {form.currency} {n(form.outstandingBalance).toLocaleString()} on {form.currency} {n(form.propertyValue).toLocaleString()}
                </p>
              </div>
              <div className="text-right">
                <p className={`font-black text-xl ${parseFloat(ltvPreview) < 70 ? "text-emerald-600" : parseFloat(ltvPreview) < 85 ? "text-amber-600" : "text-rose-500"}`}>
                  {ltvPreview}%
                </p>
