...
export function MortgageTxnHistory({ mortgage: _mortgage, transactions, onAdd, onDelete }: MortgageTxnHistoryProps) {
  ...
  const totalPrincipal = transactions.filter((t) => t.type === "repayment").reduce((s, t) => s + (t.principal ?? 0), 0);
  const totalInterest  = transactions.filter((t) => t.type === "repayment").reduce((s, t) => s + (t.interest  ?? 0), 0);
      ...
      <div className="divide-y divide-slate-50">
        {sorted.length === 0 && (
          <p className="text-center py-10 text-slate-400 text-sm">
            No transactions. <button onClick={onAdd} className="text-indigo-500 hover:underline">Add one</button>
          </p>
        )}
        {sorted.map((t) => {
          ...
          const Icon = m.icon;
          return (
            <div key={t.id} className="flex items-center gap-3 px-5 py-3 group hover:bg-slate-50/60 transition-colors">
