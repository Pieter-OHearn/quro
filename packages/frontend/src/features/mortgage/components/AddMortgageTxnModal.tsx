...
function validateTxn(
  ...
  if (type === 'repayment' && parsedInterest > parsedAmount) return 'Interest cannot exceed total repayment';
  if (type === 'rate_change' && (parsedAmount <= 0 || parsedAmount > 25)) return 'Enter a valid interest rate (0-25%)';
  if (type === 'rate_change' && parsedFixedYears <= 0) return 'Enter the number of years the rate is fixed for';
  return '';
}
...
type ContextPillProps = { type: MortgageTxnType; mortgage: MortgageType; fmt: (n: number) => string };
...
function RepaymentPreview({ parsedAmount, parsedInterest, derivedPrincipal, mortgage, fmt }: Omit<LivePreviewProps, 'type' | 'computedFixedUntil'>) {
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex justify-between text-xs border-t border-indigo-100 pt-1.5 mt-1">
        ...
        <span className="font-semibold text-slate-800">{fmt(Math.max(0, mortgage.outstandingBalance - derivedPrincipal))}</span>
      </div>
    </div>
  );
}

function ValuationPreview({ parsedAmount, mortgage, fmt }: Pick<LivePreviewProps, 'parsedAmount' | 'mortgage' | 'fmt'>) {
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between text-xs">
      ...
      <div className="flex justify-between text-xs">
        ...
        <span className={`font-semibold ${parsedAmount >= mortgage.propertyValue ? 'text-emerald-600' : 'text-rose-500'}`}>{fmt(parsedAmount)}</span>
      </div>
      <div className="flex justify-between text-xs">
        <span className="text-slate-600">Change</span>
        <span className={`font-semibold ${parsedAmount >= mortgage.propertyValue ? 'text-emerald-600' : 'text-rose-500'}`}>
          {parsedAmount >= mortgage.propertyValue ? '+' : ''}{fmt(parsedAmount - mortgage.propertyValue)}
        </span>
      </div>
      <div className="flex justify-between text-xs border-t border-emerald-100 pt-1.5 mt-1">
        ...
        <span className="font-semibold text-slate-800">{((mortgage.outstandingBalance / parsedAmount) * 100).toFixed(1)}%</span>
      </div>
    </div>
  );
}

function RateChangePreview({ parsedAmount, computedFixedUntil, mortgage, fmt }: Pick<LivePreviewProps, 'parsedAmount' | 'computedFixedUntil' | 'mortgage' | 'fmt'>) {
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between text-xs">
      ...
      <div className="flex justify-between text-xs">
        ...
        <span className={`font-semibold ${parsedAmount <= mortgage.interestRate ? 'text-emerald-600' : 'text-rose-500'}`}>{parsedAmount}%</span>
      </div>
      <div className="flex justify-between text-xs">
        <span className="text-slate-600">Monthly interest est.</span>
        <span className="font-semibold text-slate-800">{fmt((mortgage.outstandingBalance * parsedAmount) / 100 / 12)}</span>
      </div>
      {computedFixedUntil && (
...
function LivePreview(props: LivePreviewProps) {
  ...
  const bgClass = type === 'valuation' ? 'bg-emerald-50 border-emerald-100' : type === 'rate_change' ? 'bg-amber-50 border-amber-100' : 'bg-indigo-50 border-indigo-100';
  return (
...
function useMortgageTxnModal(mortgage: MortgageType, onSave: AddMortgageTxnModalProps['onSave'], onClose: () => void) {
  ...
  const handleTypeChange = (t: MortgageTxnType) => { setType(t); setError(''); setAmount(''); setInterest(''); setFixedYears(''); };
  const handleSave = () => {
    const err = validateTxn(type, parsedAmount, parsedInterest, parsedFixedYears);
    if (err) { setError(err); return; }
    onSave({ mortgageId: mortgage.id, type, amount: parsedAmount, interest: type === 'repayment' ? parsedInterest : null, principal: type === 'repayment' ? derivedPrincipal : null, fixedYears: type === 'rate_change' ? parsedFixedYears : null, date, note });
    onClose();
  };
  return { type, amount, setAmount, interest, setInterest, fixedYears, setFixedYears, date, setDate, note, setNote, error, setError, parsedAmount, parsedInterest, parsedFixedYears, derivedPrincipal, computedFixedUntil, handleTypeChange, handleSave };
}
...
type TxnAmountFieldProps = { type: MortgageTxnType; error: string; amount: string; mortgage: MortgageType; setAmount: (v: string) => void; setError: (v: string) => void };
function TxnAmountField({ type, error, amount, mortgage, setAmount, setError }: TxnAmountFieldProps) {
  return (
    <div>
      ...
      <div className="relative">
        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs font-medium">{type === 'rate_change' ? '%' : mortgage.currency}</span>
...
type RateChangeFieldsProps = { error: string; parsedFixedYears: number; fixedYears: string; computedFixedUntil: string | null; setFixedYears: (v: string) => void; setError: (v: string) => void };
function RateChangeFields({ error, parsedFixedYears, fixedYears, computedFixedUntil, setFixedYears, setError }: RateChangeFieldsProps) {
  return (
    <div>
      <label className="block text-xs font-semibold text-slate-600 mb-1.5">Fixed Period <span className="text-slate-400 font-normal">— how long is this rate fixed for?</span></label>
      <div className="relative">
        <input type="number" step="0.5" min="0.5" max="30"
          ...
          placeholder="e.g. 2" value={fixedYears}
      ...
      {computedFixedUntil && <p className="mt-1.5 text-xs text-slate-500">Fixed until <span className="font-semibold text-amber-600">{computedFixedUntil}</span></p>}
    </div>
  );
}

type RepaymentInterestFieldProps = { error: string; parsedInterest: number; parsedAmount: number; derivedPrincipal: number; interest: string; mortgage: MortgageType; fmt: (n: number) => string; setInterest: (v: string) => void; setError: (v: string) => void };
function RepaymentInterestField({ error, parsedInterest, parsedAmount, derivedPrincipal, interest, mortgage, fmt, setInterest, setError }: RepaymentInterestFieldProps) {
  return (
    <div>
      <label className="block text-xs font-semibold text-slate-600 mb-1.5">Interest Portion ({mortgage.currency}) <span className="text-slate-400 font-normal">— principal is auto-calculated</span></label>
      <div className="relative">
        ...
        <input type="number" step="0.01"
          ...
          placeholder="0.00" value={interest}
...
type TxnModalFormBodyProps = { state: TxnModalState; mortgage: MortgageType; fmt: (n: number) => string };
function TxnModalFormBody({ state, mortgage, fmt }: TxnModalFormBodyProps) {
  const { type, amount, setAmount, interest, setInterest, fixedYears, setFixedYears, date, setDate, note, setNote, error, setError, parsedAmount, parsedInterest, parsedFixedYears, derivedPrincipal, computedFixedUntil, handleTypeChange } = state;
  return (
    <div className="p-6 space-y-5">
      <TxnTypeSelector type={type} onTypeChange={handleTypeChange} />
      <ContextPill type={type} mortgage={mortgage} fmt={fmt} />
      <TxnAmountField type={type} error={error} amount={amount} mortgage={mortgage} setAmount={setAmount} setError={setError} />
      {type === 'rate_change' && <RateChangeFields error={error} parsedFixedYears={parsedFixedYears} fixedYears={fixedYears} computedFixedUntil={computedFixedUntil} setFixedYears={setFixedYears} setError={setError} />}
      {type === 'repayment' && <RepaymentInterestField error={error} parsedInterest={parsedInterest} parsedAmount={parsedAmount} derivedPrincipal={derivedPrincipal} interest={interest} mortgage={mortgage} fmt={fmt} setInterest={setInterest} setError={setError} />}
      {error && <p className="text-xs text-rose-500">{error}</p>}
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-semibold text-slate-600 mb-1.5">Date</label>
          <input type="date" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300" value={date} onChange={(e) => setDate(e.target.value)} />
        </div>
        <div>
          ...
          <input className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300" placeholder="e.g. Monthly repayment…" value={note} onChange={(e) => setNote(e.target.value)} />
        </div>
      </div>
      <LivePreview type={type} parsedAmount={parsedAmount} parsedInterest={parsedInterest} derivedPrincipal={derivedPrincipal} computedFixedUntil={computedFixedUntil} mortgage={mortgage} fmt={fmt} />
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-gradient-to-r from-[#0a0f1e] to-[#1a1f3e] px-6 py-5 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-white/10 text-slate-400 hover:text-white transition-colors"><X size={18} /></button>
        </div>
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex gap-3">
          ...
          <button onClick={state.handleSave} className="flex-1 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 text-sm transition-colors font-medium">Record</button>
        </div>
      </div>
    </div>
  );
