...
export function AddMortgageTxnModal({ mortgage, onClose, onSave }: AddMortgageTxnModalProps) {
  ...
  const handleSave = () => {
    ...
    if (type === 'repayment' && parsedInterest > parsedAmount) {
      setError('Interest cannot exceed total repayment');
      return;
    }
    if (type === 'rate_change' && (parsedAmount <= 0 || parsedAmount > 25)) {
      setError('Enter a valid interest rate (0-25%)');
      return;
    }
    if (type === 'rate_change' && parsedFixedYears <= 0) {
      setError('Enter the number of years the rate is fixed for');
      return;
    }
    onSave({
            ...
            {type === 'repayment' && (
              <>
                ...
                <span className="font-semibold text-slate-800">
                  {fmt(mortgage.outstandingBalance)}
                </span>
              </>
            )}
            {type === 'valuation' && (
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">
                ...
                <span className="text-slate-400 font-normal">
                  — how long is this rate fixed for?
                </span>
              </label>
              <div className="relative">
                <input
