...
import {
  X, Eye, EyeOff, ArrowRight, Check, BarChart2, PiggyBank,
  TrendingUp, Home, Target, Wallet, ShieldCheck, Briefcase,
  Globe, Lock, Sparkles, ChevronRight, Menu,
} from "lucide-react";
...
function SignInModal({ onClose, onSwitchToSignUp }: { onClose: () => void; onSwitchToSignUp: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-gradient-to-br from-[#0a0f1e] to-[#1a2550] px-8 pt-8 pb-10 text-center relative">
          <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-xl hover:bg-white/10 text-slate-400 hover:text-white transition-colors">
            <X size={16} />
          </button>
        ...
        <div className="px-8 py-7 space-y-4 text-slate-900">
          ...
          <p className="text-center text-sm text-slate-500 pt-1">
            ...
            <button onClick={onSwitchToSignUp} className="text-indigo-600 font-semibold hover:text-indigo-800 transition-colors">
              Sign up free
            </button>
          </p>
        </div>
...
function SignUpModal({ onClose, onSwitchToSignIn }: { onClose: () => void; onSwitchToSignIn: () => void }) {
  const navigate = useNavigate();
        ...
        <div className="px-8 py-7 space-y-4 overflow-y-auto text-slate-900">
            ...
            {form.password && (
              <div className="mt-2 flex items-center gap-2">
                <div className="flex gap-1 flex-1">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className={`h-1 flex-1 rounded-full transition-all ${i <= strengthScore ? strengthColor : "bg-slate-200"}`} />
                  ))}
                </div>
                <span className={`text-[10px] font-semibold ${strengthScore <= 1 ? "text-rose-500" : strengthScore === 2 ? "text-amber-500" : strengthScore === 3 ? "text-blue-500" : "text-emerald-600"}`}>
                  {strengthLabel}
                </span>
              </div>
            )}
          ...
          >
            {loading ? (
              ...
              <><Sparkles size={15} /><span>Create Free Account</span></>
            )}
          </button>

          <p className="text-center text-sm text-slate-500 pb-1">
            Already have an account?{" "}
            <button onClick={onSwitchToSignIn} className="text-indigo-600 font-semibold hover:text-indigo-800 transition-colors">
              Sign in
            </button>
          </p>
        </div>
...
const features = [
  { icon: BarChart2,   label: "Dashboard",   desc: "Your entire financial picture in one beautiful, real-time overview.",                          color: "bg-indigo-50 text-indigo-600",   border: "hover:border-indigo-200" },
  { icon: PiggyBank,   label: "Savings",     desc: "Track Easy Access accounts and Term Deposits across multiple banks.",                          color: "bg-emerald-50 text-emerald-600", border: "hover:border-emerald-200" },
  { icon: TrendingUp,  label: "Investments", desc: "Monitor your brokerage portfolio and property investments with P&L tracking.",                 color: "bg-violet-50 text-violet-600",   border: "hover:border-violet-200" },
  { icon: Home,        label: "Mortgage",    desc: "Log repayments, valuations, and rate changes with a live amortisation view.",                   color: "bg-sky-50 text-sky-600",         border: "hover:border-sky-200" },
  { icon: ShieldCheck, label: "Pension",     desc: "Manage multiple pension pots across countries — ABP, Australian Super, SIPP.",                 color: "bg-amber-50 text-amber-600",     border: "hover:border-amber-200" },
  { icon: Briefcase,   label: "Salary",      desc: "Record monthly payslips and track gross-to-net across your career.",                           color: "bg-rose-50 text-rose-600",       border: "hover:border-rose-200" },
  { icon: Target,      label: "Goals",       desc: "Set savings targets and watch your progress towards each milestone.",                          color: "bg-teal-50 text-teal-600",       border: "hover:border-teal-200" },
  { icon: Wallet,      label: "Budget",      desc: "Plan monthly spending by category and see where every euro goes.",                             color: "bg-orange-50 text-orange-600",   border: "hover:border-orange-200" },
];
    ...
    <nav className="sticky top-0 z-40 border-b border-white/10 bg-[#0a0f1e]/90 backdrop-blur-md">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          <button onClick={onSignIn} className="text-sm font-medium text-slate-300 hover:text-white px-4 py-2 rounded-xl hover:bg-white/5 transition-all">
            Sign In
          </button>
          <button onClick={onSignUp} className="text-sm font-semibold bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2 rounded-xl transition-all shadow-lg shadow-indigo-600/30">
            Get Started
          </button>
        </div>

        <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden p-2 rounded-xl hover:bg-white/5 text-slate-400 hover:text-white">
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-white/10 bg-[#0a0f1e] px-6 py-4 space-y-3">
          {["Features", "How it works"].map((l) => (
            <a key={l} href={`#${l.toLowerCase().replace(/\s+/g, "-")}`} className="block text-sm text-slate-400 hover:text-white py-1 transition-colors">{l}</a>
          ))}
          <div className="flex flex-col gap-2 pt-2 border-t border-white/10">
            <button onClick={() => { onSignIn(); setMenuOpen(false); }} className="text-sm font-medium text-white border border-white/20 py-2.5 rounded-xl hover:bg-white/5 transition-all">Sign In</button>
            <button onClick={() => { onSignUp(); setMenuOpen(false); }} className="text-sm font-semibold bg-indigo-600 hover:bg-indigo-500 text-white py-2.5 rounded-xl transition-all">Get Started Free</button>
          </div>
        </div>
      )}
    </nav>
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-20 lg:py-28 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="text-slate-400 text-lg leading-relaxed mb-8 max-w-lg">
              Quro unifies your savings, investments, mortgage, pension, salary and budget in one elegant dashboard — with full multi-currency support for wherever life takes you.
            </p>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-14">
            ...
            <p className="text-slate-500 max-w-lg mx-auto">
              Stop juggling spreadsheets. Quro gives you a dedicated tracker for every part of your financial life.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {features.map(({ icon: Icon, label, desc, color, border }) => (
              <div key={label} className={`bg-white border border-slate-100 rounded-2xl p-5 hover:shadow-lg transition-all duration-200 cursor-default ${border}`}>
      ...
      <section id="how-it-works" className="bg-slate-50 text-slate-900 py-20">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { step: "01", title: "Create your account", desc: "Sign up in seconds — no credit card, no subscriptions, no nonsense.", icon: Sparkles, color: "text-indigo-600 bg-indigo-50" },
              { step: "02", title: "Add your assets",     desc: "Enter your savings, investments, mortgage, salary and pension details.", icon: BarChart2, color: "text-violet-600 bg-violet-50" },
              { step: "03", title: "Watch your wealth grow", desc: "Get a live view of your net worth, goals progress, and spending trends.", icon: TrendingUp, color: "text-emerald-600 bg-emerald-50" },
      ...
      <section className="bg-white text-slate-900 py-20 border-t border-slate-100">
        <div className="max-w-5xl mx-auto px-6">
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: Globe,   title: "Multi-currency",      desc: "Track assets in EUR, AUD, USD, GBP and more. Set your base currency and every figure converts automatically.", color: "bg-indigo-600" },
              { icon: Lock,    title: "Secure by design",    desc: "Session-based authentication with hashed passwords. Your financial data is protected behind your personal account.", color: "bg-violet-600" },
              { icon: Sparkles, title: "Beautifully crafted", desc: "Clean, fast, and responsive. A personal finance tool that you'll actually enjoy opening every day.",              color: "bg-purple-600" },
