...
export function AddPropertyTxnModal({ property, mortgageBalance, onClose, onSave }: AddPropertyTxnModalProps) {
  ...
  function handleSave() {
    ...
    if (type === "repayment" && mortgageBalance <= 0) {
      setError("Link a mortgage to record repayments");
      return;
    }
    if (type === "repayment" && parsedInterest < 0) {
      setError("Interest cannot be negative");
      return;
    }
  ...
  return (
    ...
    >
      ...
      <FormField
        label={
          type === "valuation"
            ? `New Estimated Value (${property.currency})`
            : type === "repayment"
              ? `Total Repayment Amount (${property.currency})`
              : type === "rent_income"
                ? `Rent Received (${property.currency})`
                : `Property Expense (${property.currency})`
        }
