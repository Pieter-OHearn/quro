...
export function PensionTxnHistory({ pot, transactions, onAdd, onDelete }: PensionTxnHistoryProps): JSX.Element {
  ...
  const employerContribs = potTxns
    .filter((t) => t.type === "contribution" && t.isEmployer)
    .reduce((s, t) => s + t.amount, 0);
  const totalFees = potTxns
    .filter((t) => t.type === "fee")
    .reduce((s, t) => s + t.amount, 0);

  const stats = [
    { label: "Total Contributions", value: `+${fmtNative(totalContributions, pot.currency, true)}`, color: "text-emerald-600" },
    { label: "Employee",            value: fmtNative(employeeContribs, pot.currency, true),          color: "text-slate-800" },
    { label: "Employer",            value: fmtNative(employerContribs, pot.currency, true),          color: "text-indigo-600" },
    { label: "Total Fees",          value: `\u2212${fmtNative(totalFees, pot.currency, true)}`,      color: "text-rose-500" },
  ];
      ...
      {sorted.map((t) => {
        ...
        return (
          <TxnRow
            ...
            date={t.date}
            badge={t.isEmployer ? { text: "Employer", className: "bg-indigo-100 text-indigo-600" } : undefined}
            amount={
              <div className="text-right flex-shrink-0">
                <p className={`text-sm font-semibold ${isFee ? "text-rose-500" : "text-emerald-600"}`}>
                  {isFee ? "\u2212" : "+"}{fmtNative(t.amount, pot.currency, true)}
                </p>
              </div>
            }
