...
export function StatCard({ label, value, subtitle, icon: Icon, color, change, href, className }: StatCardProps) {
  const content = (
    <>
  ...
  if (href) {
    return (
      <Link to={href} className={cn("bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-shadow group", className)}>
        {content}
      </Link>
    );
  }
